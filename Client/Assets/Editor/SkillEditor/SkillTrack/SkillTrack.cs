...
public class SkillTrack : MonoBehaviour, IDirectable
{
 ...
 type.GetCustomAttributes(typeof(CategoryAttribute), true).FirstOrDefault() as CategoryAttribute;
            ...
            var newAction = gameObject.AddComponent(type) as SkillActionClip;
    ...
    {
        SkillEditorUtility.onSelectionChange += OnDirectableSelectionChange;
    }
    ...
    {
        if (directable is IKeyable && directable.parent is SkillTrack)
        {
            (directable.parent as SkillTrack).showCurvesClip = (IKeyable) directable;
        }
    }
    ...
    virtual public Texture icon
    {
        get
        {
            if (_icon == null)
            {
                var att = this.GetType().RTGetAttribute<Slate.IconAttribute>(true);
                if (att != null)
                {
    ...
    public SkillActionClip AddAction(System.Type type, float time)
    {
        ...
        root.Validate();
        SkillEditorUtility.selectedObject = newAction;
    ...
    public void DeleteAction(SkillActionClip action)
    {
        ...
        clips.Remove(action);
        if (ReferenceEquals(SkillEditorUtility.selectedObject, action))
        {
    ...
    virtual public void OnTrackInfoGUI(Rect trackRect)
    {
        ...
        if (showCurves)
        {
            ...
            GUI.enabled = true;
            DoParamsInfoGUI(e, trackRect, showCurvesClip, showCurvesClip is Slate.ActionClips.AnimateProperties);
    ...
    protected void DoDefaultInfoGUI(Event e, Rect trackRect)
    {
        ...
        if (icon != null)
        {
            ...
            GUI.color = ReferenceEquals(SkillEditorUtility.selectedObject, this) ? Color.white : new Color(1, 1, 1, 0.8f);
        ...
        GUI.color = Slate.ColorUtility.Grey(UnityEditor.EditorGUIUtility.isProSkin ? 0.2f : 1f).WithAlpha(0.2f);
    ...
    void DoTrackContextMenu(Event e, Rect clipsPosRect, float cursorTime)
    {
        if (e.type == EventType.ContextClick && clipsPosRect.Contains(e.mousePosition))
        {
            ...
            if (attachableTypeInfos.Count > 0)
            {
                ...
                var copyType = SkillEditorUtility.GetCopyType();
                if (copyType != null && attachableTypeInfos.Select(i => i.type).Contains(copyType))
                {
                    ...
                    menu.AddItem(new GUIContent(string.Format("Paste Clip ({0})", copyType.Name)), false,
                        () => { SkillEditorUtility.PasteClip(this, cursorTime); });
                }
        ...
        IKeyable keyable)
    {
        ...
        if (inspectedParameterIndex == -1)
        {
            ...
            for (var i = 0; i < keyable.animationData.animatedParameters.Count; i++)
            {
                ...
                {
                    DopeSheetEditor.DrawDopeSheet(animParam, keyable, paramRect, finalTimeRect.x, finalTimeRect.width,
                        true);
                }
                else
                {
