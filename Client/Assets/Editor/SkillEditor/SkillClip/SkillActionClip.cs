...
public class SkillActionClip : MonoBehaviour, IKeyable
{
    ...
    void IDirectable.SceneGUI(bool selected)
    {
        ...
        if (hasParameters)
        {
            ...
            for (var i = 0; i < animationData.animatedParameters.Count; i++)
            {
                ...
                var animAtt =
                    attributes.FirstOrDefault(a => a is AnimatableParameterAttribute) as AnimatableParameterAttribute;
                if (animAtt != null)
                {
    ...
    public void SetParameterEnabled<T, TResult>(System.Linq.Expressions.Expression<Func<T, TResult>> func, bool enabled)
    {
