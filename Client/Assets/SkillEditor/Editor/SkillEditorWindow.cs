...
public class SkillEditorWindow : EditorWindow
{
        ...
        public Skill Skill {
            ...
            {
                if ( _skill == null ) {
                    _skill = EditorUtility.InstanceIDToObject(_skillID) as Skill;
                }
                return _skill;
            }
            private set
        ...
        public float viewTimeMin {
            get { return Skill.viewTimeMin; }
            set { Skill.viewTimeMin = value; }
        }

        //The max view time
        public float viewTimeMax {
            get { return Skill.viewTimeMax; }
            set { Skill.viewTimeMax = value; }
        }
        ...
        private Color scruberColor {
            get { return Skill.isActive ? Color.yellow : new Color(1, 0.3f, 0.3f); }
        }
        ...
        void OnEnable() {
            ...
            UnityEditor.SceneManagement.PrefabStage.prefabStageClosing += (stage) => { if ( Skill != null && stage.IsPartOfPrefabContents(Skill.gameObject) ) { Stop(true); } };
#endif
...
#pragma warning restore
...
#endif
            Tools.hidden = false;
            if ( Skill != null && !Application.isPlaying ) {
                Stop(true);
            }
            current = null;
        }
        ...
        void InitializeAll() { InitializeAll(Skill); }
        void InitializeAll(Skill newSkill) {
        ...
        void InitClipWrappers() {
            ...
            multiSelection = null;
            var lastTime = Skill.currentTime;
            ...
            {
                for (int a = 0; a < Skill.tracks[t].clips.Count; a++)
                {
                    var id = UID(t, a);
                    ...
                    var clip = Skill.tracks[t].clips[a];
        ...
        public void Stop(bool forceRewind) {
            ...
            Skill.Sample(editorPlaybackState != EditorPlaybackState.Stoped && !forceRewind ? lastStartPlayTime : 0);
            editorPlaybackState = EditorPlaybackState.Stoped;
            willRepaint = true;
        }
        ...
        void StepForward() {
            ...
            if ( Skill.currentTime == Skill.length ) {
                Skill.currentTime = 0;
                return;
            }
            Skill.currentTime = Skill.GetPointerTimes().FirstOrDefault(t => t > Skill.currentTime + 0.01f);
        }
        ...
        void StepBackward() {
            ...
            if ( Skill.currentTime == 0 ) {
                Skill.currentTime = Skill.length;
                return;
            }
        ...
        void OnEditorUpdate() {
            ...
            Skill.currentTime += editorPlaybackState == EditorPlaybackState.PlayingForwards ? delta : -delta;
        }
        ...
        void OnSceneGUI(SceneView sceneView) {
            ...
            if ( Skill.currentTime > 0 && ( Skill.currentTime < Skill.length || !Application.isPlaying ) ) {
                ...
                GUI.color = Skill.isActive ? Color.green : Color.red;
                ...
                if ( Skill.isActive ) {
                    GUI.Label(bottom, string.Format(" Active '{0}'", Skill.name), GUIStyle.none);
                } else {
                    GUI.Label(bottom, string.Format(" Previewing '{0}'. Non animatable changes made to actor components will be reverted.", Skill.name), GUIStyle.none);
                }
            }
        ...
        void DrawGuides() {
            ...
            if ( Skill.currentTime > 0 ) {
                DrawGuideLine(Skill.currentTime, scruberColor);
            }
            ...
            if ( Skill.isActive ) {
                if ( Skill.playTimeMin > 0 ) {
                    DrawGuideLine(Skill.playTimeMin, Color.red);
                }
                if ( Skill.playTimeMax < length ) {
                    DrawGuideLine(Skill.playTimeMax, Color.red);
                }
            }
        ...
        void DoScrubControls() {
            ...
            if ( Skill.isActive ) { //no scrubbing if playing in runtime
                return;
            }
            ...
            if ( e.type == EventType.MouseDown && topMiddleRect.Contains(mousePosition) ) {
                ...
                if ( e.button == 1 && isEndCarret && Skill.directables != null ) {
                    var menu = new GenericMenu();
            ...
            if ( isMovingScrubCarret ) {
                Skill.currentTime = SnapTime(pointerTime);
                Skill.currentTime = Mathf.Clamp(Skill.currentTime, Mathf.Max(viewTimeMin, 0) + float.Epsilon, length - float.Epsilon);
            }
        ...
        void ShowPlaybackControls(Rect topLeftRect) {
            ...
            var isStoped = Application.isPlaying ? ( Skill.isPaused || !Skill.isActive ) : editorPlaybackState == EditorPlaybackState.Stoped;
            if ( isStoped ) {
        ...
        void ShowTimeInfo(Rect topMiddleRect) {
            ...
            GUI.BeginGroup(topMiddleRect);
            {
                ...
                if ( Skill.currentTime > 0 ) {
                    var label = doFrames ? ( Skill.currentTime * Prefs.frameRate ).ToString("0") : Skill.currentTime.ToString("0.00");
                    var text = "<b><size=17>" + label + "</size></b>";
                    var size = Styles.headerBoxStyle.CalcSize(new GUIContent(text));
                    var posX = TimeToPos(Skill.currentTime);
                    var stampRect = new Rect(0, 0, size.x, size.y);
